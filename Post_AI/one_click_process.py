...
class Make_3d_Modeling():
    def __init__(self, filepath, is_bool, token):
        ...
        self.token = token
        self.python_cmd = '/home/metaai/anaconda3/envs/magic123/bin/python'
...
for i in range(len(input_images)):
    ...
    new_folder = input_images[i].split('/')[-1].split('.')[0]
    os.makedirs(f'./run_results/{new_folder}', exist_ok=True)
    mesh_path = f'./run/{new_folder}/outputs/retouch/mesh'
    mp4_path = f'./run/{new_folder}/outputs/retouch/results'

    to_path = f'./run_results/{new_folder}'
    shutil.copyfile(mesh_path + '/albedo.png', to_path + '/albedo.png')
    shutil.copyfile(mesh_path + '/mesh.mtl', to_path + '/mesh.mtl')
    shutil.copyfile(mesh_path + '/mesh.obj', to_path + '/mesh.obj')
    shutil.copyfile(mp4_path + '/retouch_ep0050_lambertian.mp4', to_path + '/example.mp4')
